...
% 2015.
function results = BASELINE_setupSimVehicle
...
% %% Test with our bus set up
% % set up cell: capacity [Ah], weight [g], (vmax, vnom, vmin) [V]
% cell = setupCell(2.1507, 50, 4.2, 3.8, 3.0); % about 50 grams for an 18650 Li ion cell
% % set up module: numParallel, numSeries, overhead for this cell by weight
% module = setupModule(280, 16, 0.08, cell);
% 
% % set up pack: numSeries, overhead by weight, (full SOC, empty SOC) [%],
...
%%
  ...
  for theCycle = 1:length(files),
    ...
    results{theCycle} = BASELINE_simVehicle(vehicle,cycle,0.3); %#ok<AGROW>
